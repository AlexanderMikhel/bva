...
var (
	mockMassTransactionRequest = bovasdk.MassTransactionRequest{
	...
	mockMassTransactionResponse = bovasdk.MassTransactionResponse{
		ResultCode: "ok",
		Payload: struct {
			ID                string `json:"id"`
...
func TestCreateMassTransaction(t *testing.T) {
	...
	mt := bovasdk.massTransactionNew(server.URL, apiSecret, server.Client())
